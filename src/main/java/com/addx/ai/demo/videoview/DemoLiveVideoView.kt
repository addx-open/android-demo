...
open class  DemoLiveVideoView: DemoBaseVideoView, OnRockerPositionChangeListener,IAddxLiveView,
    AddxLiveOptListener {
...
//        LogUtils.d(TAG, "addExtendOptBtnsByDeviceType----adminCount:$adminCount----adminRealCount:$adminRealCount----shareCount:$shareCount----adminCount:$shareRealCount")
        if(itemCount < 4){
            ...
            if(more != null && more?.childCount!! > 0){
                for(index in 0 until more.childCount){
                    (more.getChildAt(index).layoutParams as GridLayout.LayoutParams).columnSpec =
...
//                    rl_expand_under_player_container.layoutParams.height = ViewGroup.LayoutParams.WRAP_CONTENT
    ...
    override fun refreshLightUI(ret: Boolean, isOpen: Boolean){
    ...
    override fun refreshRingUI(ret: Boolean, isOpen: Boolean){
